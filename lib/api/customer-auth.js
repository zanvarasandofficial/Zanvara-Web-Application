...
export function getGoogleAuthUrl(redirectPath = "/checkout") {
  ...
  return `${apiBase}/api/auth/google?${params.toString()}`;
}

export async function fetchCustomerProfile(explicitToken) {
  const token = explicitToken ?? getCustomerToken();
  ...
  try {
    ...
    throw new Error("Could not reach Zanvara API. Check your connection and try again.");
  }
