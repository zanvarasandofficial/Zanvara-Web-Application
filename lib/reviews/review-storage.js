...
function hasUserSubmittedReview(productId, userId, userEmail) {
  if (!productId) return false;

  return readUserSubmittedReviews().some((review) => {
    if (review.productId !== productId || review.source === "admin") {
      return false;
    }
    ...
    if (userEmail && review.userEmail) {
      return (
        String(review.userEmail).toLowerCase() === String(userEmail).toLowerCase()
      );
    }
...
export function hasUserReviewedProduct(productId, userId, userEmail) {
  return hasUserSubmittedReview(productId, userId, userEmail);
}

function evaluateReviewEligibility(orders, productId, userId = null, userEmail = null) {
  if (hasUserSubmittedReview(productId, userId, userEmail)) {
    return {
      canReview: false,
      state: "already_reviewed",
      message: "Aap is product ka review pehle hi submit kar chuke hain.",
    };
  }
...
export function getReviewEligibility(productId, orders = null, userId = null, userEmail = null) {
  const orderList = orders ?? readOrderHistory();
  return evaluateReviewEligibility(orderList, productId, userId, userEmail);
}

export async function fetchReviewEligibility(productId, userId = null, userEmail = null) {
  try {
    ...
    return getReviewEligibility(productId, null, userId, userEmail);
  }
}
