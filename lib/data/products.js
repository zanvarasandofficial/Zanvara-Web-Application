...
export const productCategories = categoryHighlights.map((category) => category.name);
...
const catalogEntries = [
  ...
  ...popularProducts.map((product, index) => ({
    ...product,
    category: ["Fashion", "Electronics", "Fashion", "Home & Living", "Beauty", "Electronics"][index],
  })),
  ...latestProducts.map((product, index) => ({
    ...product,
    category: ["Home & Living", "Electronics", "Sports", "Home & Living", "Sports", "Beauty"][index],
  })),
  ...bestDeals.map((product, index) => ({
    ...product,
    category: ["Electronics", "Fashion", "Electronics", "Fashion", "Fashion", "Electronics"][index],
  })),
];

export const allProducts = catalogEntries
  .filter(
    (product, index, list) =>
  ...
  .map((product, index) => ({
    ...product,
    stock: 6 + ((index * 11 + product.name.length * 3) % 45),
    description:
      product.description ||
      `${product.name} — curated by Zanvara with premium quality and fast delivery across Pakistan.`,
  }));
...
export const catalogPriceBounds = allProducts.reduce(
  (bounds, product) => ({
